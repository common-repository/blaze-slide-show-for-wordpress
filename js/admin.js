...
//add replace on fileObj.filePath.replace("/wordpress/wp-admin/","") to workaround on windows server
	...
	test=file;
	var params = 'myajax=1&filename='+ blaze_upload_dir+"/"+file.name + '&task=blaze_resize_image_and_add&album_id='+jQuery('#multi_album_id').val();
...
function on_queue_completed(file)
{
	...
	blaze_refresh_images_table(blaze_current_album_id);
...
function blaze_refresh_albums_table()
{
	...
	//sleep(2000);
	jQuery.post('index.php', 'task=blaze_get_albums_table', function(res)
	{
		blaze_set_data_tables();
		if (res.status == 'ok') 
		{
...
function blaze_refresh_images_table(album_id)
{
	...
	jQuery.post('index.php', 'task=blaze_get_albums_images_table&album_id='+album_id, function(res)
	{
		blaze_set_data_tables();
		if (res.status == 'ok') 
		{
...
function re_order()
{
	...
	if( task == 'blaze_reorder_album' )
		jQuery('#albums_table tbody').html('<tr><td colspan="7" align="center"><img src="'+loading.src+'" alt="" /><br/>Please wait, refreshing albums</td></tr>');
	if( task == 'blaze_reorder_image' )
	...
	jQuery.get('index.php?' + params, function(res)
	{
		if( task == 'blaze_reorder_image' )
		{
			blaze_refresh_images_table(album_id);
		}
		if( task == 'blaze_reorder_album' )
			blaze_refresh_albums_table();
...
var tpager = null;
var ipager = null;
function blaze_set_data_tables()
{
	//add event to bulk actions
	jQuery('.bulk_form').live('submit', blaze_submit_bulk);
...
function blaze_submit_bulk()
{
	var task = this.task.value;
	var album_id = null;
	if( task == 'blaze_bulk_delete_albums' || task == 'blaze_bulk_enable_albums' || task == 'blaze_bulk_disable_albums')
	{
	...
	else if( task == 'blaze_bulk_delete_images' || task == 'blaze_bulk_enable_images' || task == 'blaze_bulk_disable_images' )
	{
...
{
	//preload images
	loading.src = blaze_url + '/images/loading.gif';
...
/*
		var ops = {
					'uploader'		: blaze_url + '/js/uploadify-2.1.4/uploadify.swf',
					'script'		: blaze_url + '/js/uploadify-2.1.4/uploadify.php',
					'cancelImg' 	: blaze_url + '/js/uploadify-2.1.4/cancel.png',
				    'folder'    	: blaze_upload_dir,
				    'auto'			: true,
					'multi'			: true,
					'fileExt'        : '*.jpg;*.gif;*.png',
...
*/
		var swfu;
			...
			var settings = {
				...
				upload_url: blaze_url+"/js/swfupload/js/upload.php",
				post_params: {"PHPSESSID" : "<?php echo session_id(); ?>","folder":blaze_upload_dir, "secw":blaze_secw, nopar: "n" + new Date().getTime()},
				...
				// Button settings
				button_image_url: blaze_url+"/js/swfupload/js/TestImageNoText_65x29.png",
				button_width: "130",
				button_height: "29",
	...
	jQuery('.blaze_delete_album').live('click', blaze_delete_album);
	...
	jQuery('table#album_images .enable, table#albums_table .enable').live('click', blaze_enable);
	jQuery('table#album_images .disable, table#albums_table .disable').live('click', blaze_disable);
